...
def ubcf_recommended_books_knn(user_id, ratings, top_n=10, k_neighbors=50):
    ...
    user_item_matrix, user_indices, book_indices = create_user_item_matrix_sparse(ratings)
    ...
    for book_idx,score_pair in predicted_scores.items():
        if score_pair[1]>0:
            book_isbn = book_indices[book_idx]
...
def ubcf_predict_for_rmse(user_id, ratings, k_neighbors=50):
    ...
    for book_idx in rated_books:
        ...
        if denominator > 0:
            ...
            predicted_scores[book_indices[book_idx]] = max(1.0, min(10.0, pred))
