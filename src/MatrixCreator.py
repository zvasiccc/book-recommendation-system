...
def prepare_data_for_matrix_sparse(ratings):
    ...
    return rows,cols,data, book_index, user_index

def create_user_item_matrix_sparse(ratings):

    rows,cols,data, book_index, user_index = prepare_data_for_matrix_sparse(ratings)
...
def create_item_user_matrix_sparse(ratings):
    ...
    item_user_matrix = csr_matrix((data, (cols, rows)), shape=(len(book_index), len(user_index)))
